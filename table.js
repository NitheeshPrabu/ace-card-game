...
function Table(tableID, playerLimit){
	this.id = tableID;
	this.name = "";
	...
	this.playerLimit = parseInt(playerLimit);
	this.pack = [];
	this.cardsOnTable = [];
...
Table.prototype.progressRound = function(player) {
	...
	while (this.players[id].hand.length == 0) {
		id = (id+1)%this.players.length;	
	}
	...
	if (this.status === "available") {
		...
		if(!found){
			...
			if(this.players.length == this.playerLimit){
				...
				if (this.playerLimit == this.players.length)
					this.status = "full";
			}
			return true;
		}
